...
export default function VlogsSection() {
  ...
  const facebooks = displayedVlogs.filter(v => v.platform === 'facebook');
  ...
  const VideoCard = ({ vlog }: { vlog: typeof vlogs[0] }) => (
    ...
    >
      {/* Video Container */}
      <div className="relative w-full bg-black/50 flex-grow flex items-center justify-center min-h-[300px]">
        {vlog.embedUrl ? (
          <iframe
            width="100%"
            height="100%"
      ...
      <div className="p-4 flex-shrink-0">
        ...
        <Link href={vlog.url} target="_blank">
          <button className={`w-full flex items-center justify-center gap-2 py-2 rounded-lg font-semibold transition text-sm ${
            vlog.platform === 'tiktok'
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-gray-50 to-gray-100 dark:from-gray-900/50 dark:to-gray-900">
