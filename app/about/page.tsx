...
export const metadata: Metadata = {
  ...
  keywords: ['Dr. Shekhar Poudel', 'gastroenterologist', 'hepatologist', 'AIIMS', 'LTSI'],
};
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-white dark:from-emerald-950/20 dark:via-green-900/20 dark:to-gray-900">
        ...
        <div className="grid md:grid-cols-3 gap-12 mb-20">
            ...
            <div className="bg-gradient-to-br from-emerald-200/40 to-teal-200/40 rounded-3xl p-8">
              <div className="w-full bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg overflow-hidden">
                <div className="text-center">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-white">{doctorProfile.name}</h2>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-300 dark:border-gray-600">
                    ...
                    <p className="text-emerald-600 dark:text-emerald-400 font-bold mt-2">{doctorProfile.mobile}</p>
                    <p className="text-sm text-gray-600 dark:text-gray-300">{doctorProfile.phone}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="md:col-span-2">
            <h2 className="text-3xl font-bold bg-gradient-to-r from-emerald-600 to-green-600 dark:from-emerald-400 dark:to-green-400 bg-clip-text text-transparent mb-6">Professional Journey</h2>
            ...
            <div className="space-y-4 mb-8">
              {doctorProfile.achievements.map((achievement, index) => (
                <div key={index} className="flex items-start space-x-4 bg-gradient-to-r from-emerald-50 to-green-50 dark:from-emerald-900/30 dark:to-green-900/30 p-4 rounded-lg border border-emerald-200 dark:border-emerald-700/50">
                  <Award size={24} className="text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-1" />
                  <div>
        ...
        <div className="mb-20">
          ...
          <div className="space-y-4">
            {doctorProfile.qualifications.map((qual, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 p-6 rounded-lg border-l-4 border-emerald-500">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-gray-900 dark:text-white text-lg">{qual.degree}</h3>
                  {qual.year && <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{qual.year}</span>}
                </div>
                <p className="text-gray-600 dark:text-gray-300">{qual.institution}</p>
                {qual.achievement && (
                  <p className="text-emerald-600 dark:text-emerald-400 text-sm mt-2">{qual.achievement}</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {doctorProfile.experience.map((exp, index) => (
              <div key={index} className="bg-white dark:bg-gray-800 p-6 rounded-lg border-l-4 border-emerald-500">
                <div className="flex justify-between items-start mb-2">
                  <h3 className="font-semibold text-gray-900 dark:text-white text-lg">{exp.position}</h3>
                  <span className="text-emerald-600 dark:text-emerald-400 text-sm font-semibold">{exp.period}</span>
                </div>
                <p className="text-gray-600 dark:text-gray-300 font-semibold">{exp.institution}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400">{exp.location}</p>
                {exp.achievement && (
                  <p className="text-emerald-600 dark:text-emerald-400 text-sm mt-2">✓ {exp.achievement}</p>
                )}
              </div>
            ))}
          </div>
